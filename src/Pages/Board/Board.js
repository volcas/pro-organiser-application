...
class Board extends Component {
  ...
  componentDidMount() {
    let url =
      "https://pro-organizer-f83b5.firebaseio.com/boardData/-LuM4blPg67eyvzgAzwn/boards/" +
    ...
    Axios.get(
      "https://pro-organizer-f83b5.firebaseio.com/boardData/-LuM4blPg67eyvzgAzwn/archivedBoards.json"
    )
      .then((response) => {
        let archivedCards = [];
  ...
  addCardToDBHandler = (values) => {
    ...
    values["id"] = this.state.boardData.cards
      ? this.state.boardData.cards.slice(-1)[0].id + 1
      : 0;
  ...
  archiveCardHandler = (archived_card) => {
    ...
    let updatedCards = cards.filter((card) => {
      return card.id !== archived_card.id;
    });
    boardData.cards = updatedCards;

    Axios.put(
      "https://pro-organizer-f83b5.firebaseio.com/boardData/-LuM4blPg67eyvzgAzwn/archivedCards.json",
      archivedCards
    )
      .then((response) => {
        this.setState({
  ...
  deleteBoardHandler = () => {
    Axios.get(
      "https://pro-organizer-f83b5.firebaseio.com/boardData/-LuM4blPg67eyvzgAzwn.json"
    )
      .then((response) => {
  ...
  render() {
    ...
    if (Object.keys(this.state.boardData).length > 0) {
        ...
        columns = dataOfBoard.columns.map((column) => {
          ...
          return (
            <BoardColumn
              ...
              droppedCard={(card, column) =>
                this.droppedCardHandler(card, column)
              }
      ...
      let cardInfo =
        Object.keys(this.state.selectedCardData).length > 0 ? (
          <CardInfo
            data={this.state.selectedCardData}
            editCard={this.editCardHandler}
            archiveCard={this.archiveCardHandler}
          />
        ) : null;
      ...
      content = (
        <>
          {this.state.showModal ? (
            <Modal content={cardInfo} close={this.closeModalHandler} />
          ) : null}
          {this.state.showAddCardModal ? (
            <Modal
              content={
                <AddCard
                  members={this.state.boardData.members}
                  addCard={this.addCardToDBHandler}
                />
              }
          ...
          {this.state.showEditModal ? (
            <Modal
              content={
                <AddCard
                  members={this.state.boardData.members}
                  addCard={this.addEditedCardToDBHandler}
                  editCard={true}
              ...
              content={
                this.state.addColumnLoading ? (
                  ...
                  <>
                    ...
                    <button
                      id="CreateColumn"
                      className={createBoardStyles.CreateButton}
                      style={{ width: "auto", float: "right" }}
                      onClick={this.addColumnToDBHandler}
                    >
                      Add Column
                    </button>
                  </>
          ...
          <div className={styles.BoardHeader}>
            <p className={styles.BoardTitle}>
              {this.state.boardData.name} Board
            </p>
            <button
              className={createBoardStyles.CreateButton}
              style={{ backgroundColor: "red", width: "auto" }}
              onClick={this.deleteBoardHandler}
            >
              Delete Board
            </button>
          </div>
          <div className={styles.ColumnsContainer}>
            {columns}
            <div className={styles.AddColumn} onClick={this.addColumnHandler}>
              Add a column
            </div>
          </div>
        </>
